...
function createUserTable() {
    db.query('CREATE TABLE IF NOT EXISTS system_user (id serial PRIMARY KEY, username text UNIQUE NOT NULL, hash text NOT NULL, email text NOT NULL, permission smallint DEFAULT 3);', (err, result) => {
        if (err) { 
            console.log("ERROR:", err)
        } else {
            db.query('SELECT EXISTS(SELECT 1 FROM system_user WHERE username = $1)',["root"]).then((result)=>{
...
async function createUser(username, password, email, permission) {
    ...
    await db.query('INSERT INTO system_user (username, hash, email, permission) VALUES ($1, $2, $3, $4);', [username, hash, email, permission])
}

async function checkCredentials(username, password) {

    let { rows } = await db.query('SELECT * FROM system_user WHERE username = $1;', [username])
    if (rows.length > 0) {
...
async function changePermission(sessionUserID, username, permissionLevel) {
    ...
    if (rows.length == 2) { // one user means they are changing it for themselves
        ...
        if (rows[iSU].id == sessionUserID && rows[iSU].permission <= 1 && rows[iSU].permission > rows[iU].permission && permissionLevel < rows[iSU].permission) {
            // change
            await db.query('UPDATE system_user SET permission=$1 WHERE username=$2;', [permissionLevel, username])
        }
    }
    return 
}
...
async function changePassword(username, password) {
    ...
    await db.query('UPDATE system_user SET hash=$1 WHERE username=$2;', [hash, username])
}
